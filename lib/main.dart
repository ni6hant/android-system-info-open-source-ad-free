...
class _SystemInfoScreenState extends State<SystemInfoScreen> {
  ...
  // Holds CPU information fetched via Platform Channel from Kotlin
  // Each key matches what MainActivity.kt puts in the map
  Map<String, String>? _cpuInfo;

  // The Platform Channel must use the EXACT same name as in Kotlin
  // Any mismatch and the channel silently fails
  // Docs: https://docs.flutter.dev/platform-integration/platform-channels
  static const _cpuChannel = MethodChannel('com.ni6hant.systeminfo/cpu');
  ...
  Future<void> _fetchCpuInfo() async {
    try {
      final cpuInfo = await _cpuChannel
          .invokeMapMethod<String, String>('getCpuInfo')
          .timeout(
            // If Kotlin doesn't respond in 5 seconds something is wrong
            const Duration(seconds: 5),
            onTimeout: () => {
              'error': 'Channel timed out — Kotlin side did not respond',
            },
          );
    ...
    return InfoSection(
      ...
      children: [
        ...
        InfoCard(
          ...
          explanation:
              'The screen dimensions in density independent pixels (dp). This is the unit Flutter and Android use for layout. Using dp instead of physical pixels ensures that buttons, text, and UI elements appear the same physical size on all screens regardless of pixel density. A button that is 200dp wide will look the same size on a budget phone and a flagship.',
        ),
        ...
        InfoCard(
          ...
          explanation:
              'The actual number of pixels on the screen panel. This is the resolution manufacturers advertise. It is calculated by multiplying the logical resolution by the pixel ratio. A higher physical resolution means sharper text and images but also uses more GPU power to render.',
        ),
        ...
        InfoCard(
          ...
          explanation:
              'The number of physical pixels per logical pixel. A ratio of 3.0 means there are 3x3 physical pixels for every 1x1 logical pixel. Higher ratios produce sharper displays. This is what Android uses to classify screens into density buckets like hdpi, xhdpi, and xxhdpi. Most modern flagships are between 2.5x and 4x.',
        ),
        ...
        InfoCard(
          ...
          value: mediaQuery.orientation == Orientation.portrait
              ...
              : 'Landscape',
          explanation:
              'Whether the device is currently in portrait or landscape mode. Portrait means the screen is taller than it is wide. Landscape means the screen is wider than it is tall. This value updates automatically when you rotate the device. Apps can choose to support one or both orientations.',
        ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('System Information')),
        ...
        InfoSection(
          ...
          children: [
            InfoCard(
              ...
              explanation:
                  'The version of Android running on this device. Android versions are named numerically since Android 10. Each version brings new features, security improvements, and API changes that apps can use.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'A number that identifies the Android framework version. Developers use this to check if a device supports specific features. For example, API 26 introduced notification channels, API 29 introduced dark mode support. Your device is on API ${_deviceInfo!.version.sdkInt}.',
            ),

            InfoCard(
              label:
                  'BaseOS (Won\'t be populated if the manufacturer didn\'t bother putting it. It\'s basically for skinned OS.)',
              value: _deviceInfo!.version.baseOS.toString(),
              explanation:
                  'The base Android version that the manufacturer built their custom OS on top of. Most manufacturers like Xiaomi, Samsung, and OnePlus ship a heavily modified version of Android. This field is often left empty by manufacturers even when it applies.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'The name of the underlying hardware circuit board. This is the physical board that all components are mounted on. It often corresponds to the chipset platform name used internally by the manufacturer.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'The version of the bootloader installed on this device. The bootloader is the first program that runs when you power on your device — it loads the Android OS. An unlocked bootloader allows installing custom ROMs and recovery images.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'The consumer-facing brand name of this device. This is the brand shown in marketing materials. For example a device manufactured by Xiaomi might have a brand of Redmi or POCO depending on which product line it belongs to.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'The development codename of this Android version. During development Google assigns alphabet-based codenames to Android releases. Once released this shows REL meaning it is a stable official release rather than a preview build.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'The CPU instruction sets this device supports, listed in order of preference. arm64-v8a is the modern 64-bit ARM architecture. armeabi-v7a is the older 32-bit ARM. x86_64 and x86 appear on emulators. Apps are compiled for these architectures so the Play Store delivers the right version for your device.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'The internal hardware codename of this device used by the manufacturer during development. This is different from the marketing name — for example a Redmi Note 12 might have a device codename of spes or tapas internally.',
            ),
            InfoCard(
              label: 'Display',
              value: _deviceInfo!.display.toString(),
              explanation:
                  'A build identifier string for the display firmware. This is used internally by Android to identify the exact build running on this device and is useful for bug reports and debugging display related issues.',
            ),
            // ── Display Section ────────────────────────────────────
            ...
            InfoCard(
              ...
              explanation:
                  'A unique string that identifies this exact build of Android on this device. It combines the brand, device, build ID, and other identifiers into one string. Developers use this to identify exactly which software version a bug report came from.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'The name of the hardware platform this device is built on. This typically corresponds to the chipset or SoC (System on Chip) platform name. For Qualcomm devices this often matches the chipset codename used internally.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'The hostname of the build server that compiled this version of Android. This is set by the manufacturer during the build process and identifies which machine in their build infrastructure produced this firmware.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'A build ID string that identifies this specific Android build. Manufacturers use this to track firmware versions for updates and support. It often appears in settings under About Phone as the Build Number.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'The internal incremental build number assigned by the manufacturer. This changes with every software update and is more granular than the Build ID. Useful for identifying the exact software version when reporting bugs to manufacturers.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'Whether Android has classified this device as a low RAM device. Android applies this classification to devices with 1GB or less of RAM. Apps can check this flag to reduce memory usage, disable animations, or offer a lite experience on low end devices.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'Whether this is a real physical Android device as opposed to an emulator running on a computer. Developers use this to disable certain features during testing or to prevent apps from running on emulators for security reasons.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'The company that manufactured the physical hardware of this device. This is the actual hardware maker which may differ from the brand. For example Foxconn manufactures devices for many brands, or Xiaomi manufactures both Xiaomi and Redmi branded devices.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'The end-user visible model name of this device as defined by the manufacturer. This is what appears in marketing materials and what users typically refer to when identifying their device. It is the most recognizable identifier for a specific device.',
            ),
            InfoCard(
              label: 'Product',
              value: _deviceInfo!.product.toString(),
              explanation:
                  'The name of the overall product that this device is part of. This is set by the manufacturer during the build process and often matches the device codename. It is used internally to identify which product configuration was used to build this firmware.',
            ),
            InfoCard(
              label: 'Name',
              value: _deviceInfo!.name.toString(),
              explanation:
                  'The device name as reported by the Android system. This may match the model name or be a slightly different internal identifier depending on the manufacturer and Android version.',
            ),
            InfoCard(
              label: 'Supported 32 Bit Abis',
              value: _deviceInfo!.supported32BitAbis.toString(),
              explanation:
                  'The 32-bit CPU instruction sets supported by this device. Even modern 64-bit devices support 32-bit instruction sets for backwards compatibility with older apps that have not been updated to 64-bit. The Play Store uses this to determine which version of an app to deliver.',
            ),
            InfoCard(
              label: 'Supported 64 Bit Abis',
              value: _deviceInfo!.supported64BitAbis.toString(),
              explanation:
                  'The 64-bit CPU instruction sets supported by this device. 64-bit apps can access more memory, run faster, and are required by Google Play for all new apps. arm64-v8a is the standard 64-bit architecture for modern Android devices.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'The API level of a preview version of Android if this device is running a pre-release build. On stable production devices this is always 0, indicating this is a final released version of Android and not a developer preview.',
            ),
            InfoCard(
              label: 'Available RAM Size',
              value: formatMB(_deviceInfo!.availableRamSize),
              explanation:
                  'The amount of RAM currently free and available for new apps and processes. Android actively manages memory by keeping recently used apps in RAM for faster switching. This number changes constantly as you open and close apps.',
            ),
            InfoCard(
              label: 'Physical RAM Size',
              value: formatMB(_deviceInfo!.physicalRamSize),
              explanation:
                  'The total physical RAM installed in this device. This is less than the advertised RAM because the Android kernel, GPU, and system processes permanently reserve memory at boot. A device advertised as 8GB will typically show around 7GB here.',
            ),
            InfoCard(
              label: 'Security Patch',
              value: _deviceInfo!.version.securityPatch.toString(),
              explanation:
                  'The date of the most recent Android security patch applied to this device. Google releases monthly security patches that fix vulnerabilities. Manufacturers then integrate these patches into their own firmware updates. A more recent date means fewer known security vulnerabilities.',
            ),
            InfoCard(
              label: 'Free Disk Size',
              value: formatBytes(_deviceInfo!.freeDiskSize),
              explanation:
                  'The amount of internal storage currently available for new files and app installations. This decreases as you install apps, take photos, and download files. Android may reserve some space and not make it available to apps even when it appears free.',
            ),
            InfoCard(
              label: 'Total Disk Size',
              value: formatBytes(_deviceInfo!.totalDiskSize),
              explanation:
                  'The total internal storage capacity of this device. This is usually less than the advertised storage because the Android OS, pre-installed apps, and system partitions occupy space before you ever use the device. Manufacturers advertise raw storage while Android reports usable storage.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'Build tags that describe the type of this Android build. release-keys means this is an official signed build from the manufacturer. test-keys indicates a development or custom ROM build. This is one way to detect if a device is running official firmware.',
            ),
            InfoCard(
              label: 'Type',
              value: _deviceInfo!.type.toString(),
              explanation:
                  'The type of this Android build. user means a production build intended for end users with full security restrictions. userdebug is a debug build that still has most security restrictions. eng is a fully open engineering build used during development.',
            ),
          ],
        ),
        ...
        InfoSection(
          ...
          children: [
            ...
            InfoCard(
              ...
              explanation:
                  'The current battery charge level as a percentage from 0 to 100. Android reads this from the battery management IC via the power supply subsystem. The value updates automatically as the battery charges or discharges. Some manufacturers report slightly inaccurate percentages due to battery calibration differences.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'The current charging state of the battery. charging means power is connected and battery is increasing. discharging means running on battery power. full means connected to power and fully charged. connectedNotCharging means power is connected but battery is not increasing, which can happen when the battery is at capacity or the charger output is too low for the current usage.',
            ),
          ],
        ),
          ...
          children: [
            ...
            InfoCard(
              ...
              value: _connectivityResult != null
                  ? _connectivityResult!.map((r) => r.name).join(', ')
                  : 'Unknown',
              explanation:
                  'The type of network connection currently active on this device. wifi means connected to a wireless network. mobile means using cellular data. ethernet applies to devices connected via a cable. vpn appears when a VPN is active. Multiple types can be active simultaneously.',
            ),
            ...
            InfoCard(
              ...
              explanation:
                  'The name of the WiFi network this device is currently connected to. SSID stands for Service Set Identifier — it is the human readable name you see when browsing available networks. Android requires location permission to read this value because WiFi SSIDs can be used to determine your physical location.',
            ),
            InfoCard(
              label: 'WiFi BSSID',
              value: _wifiBSSID ?? 'Unavailable',
              explanation:
                  'The MAC address of the specific WiFi access point this device is connected to. BSSID stands for Basic Service Set Identifier. Unlike the SSID which is a name, the BSSID is a unique hardware identifier for the router or access point. Multiple access points can share the same SSID but each has a unique BSSID.',
            ),
            InfoCard(
              label: 'IPv4 Address',
              value: _wifiIPv4 ?? 'Unavailable',
              explanation:
                  'The IPv4 address assigned to this device on the current network. This is a 32-bit address written as four numbers separated by dots, for example 192.168.1.5. This address is local to your network and changes when you connect to different networks. Other devices on the same network use this address to communicate with yours.',
            ),
            InfoCard(
              label: 'IPv6 Address',
              value: _wifiIPv6 ?? 'Unavailable',
              explanation:
                  'The IPv6 address assigned to this device. IPv6 is the newer 128-bit addressing system designed to replace IPv4 which is running out of available addresses. IPv6 addresses are written as eight groups of four hexadecimal digits. Many networks now assign both IPv4 and IPv6 addresses simultaneously.',
            ),
            InfoCard(
              label: 'Gateway',
              value: _wifiGateway ?? 'Unavailable',
              explanation:
                  'The IP address of your router or access point. The gateway is the device that connects your local network to the internet. All traffic destined for the internet is sent to this address first, which then forwards it to the correct destination. It is typically the first address in your network range.',
            ),
            InfoCard(
              label: 'Subnet Mask',
              value: _wifiSubmask ?? 'Unavailable',
              explanation:
                  'A number that defines which part of an IP address identifies the network and which part identifies the device. A common subnet mask of 255.255.255.0 means the first three numbers identify the network and the last number identifies the specific device. It determines which devices are on the same local network.',
            ),
            InfoCard(
              label: 'Wifi Broadcast',
              value: _wifiBroadcast ?? 'Unavailable',
              explanation:
                  'The broadcast address for the current network. Messages sent to this address are received by all devices on the same network simultaneously. It is calculated from the IP address and subnet mask combined. Network discovery and local service announcements use this address.',
            ),
            ...
            if (_networkError != null)
              InfoCard(
                ...
                value: _networkError!,
                explanation:
                    'This is a temporary debug card showing the exact error from the WiFi fetch. This will be removed once the issue is identified and fixed.',
              ),
          ],
        ),
          ...
          children: _cpuInfo == null
              ...
              : [
                  InfoCard(
                    ...
                    value: _cpuInfo!['availableCores'] ?? 'Unavailable',
                    explanation:
                        'The number of CPU cores currently active and available to Android. Modern phones use a technique called CPU hotplugging where cores are turned off when not needed to save battery. This number may be lower than the total core count when the device is idle.',
                  ),
                  InfoCard(
                    label: 'Total Cores',
                    value: _cpuInfo!['totalCores'] ?? 'Unavailable',
                    explanation:
                        'The total number of CPU cores in this device including any that are currently offline to save power. Modern Android chips use a big.LITTLE architecture with fast performance cores and efficient battery saving cores working together.',
                  ),
                  InfoCard(
                    label: 'Max Frequency',
                    value: _cpuInfo!['maxFrequency'] ?? 'Unavailable',
                    explanation:
                        'The maximum clock speed this CPU can run at under full load. Higher frequency means faster processing but also more heat and battery drain. Modern chips boost to maximum frequency only when needed and throttle back when idle or hot.',
                  ),
                  InfoCard(
                    label: 'Min Frequency',
                    value: _cpuInfo!['minFrequency'] ?? 'Unavailable',
                    explanation:
                        'The minimum clock speed this CPU runs at when idle. Running at lower frequencies saves significant battery. The difference between min and max frequency shows the range the CPU governor can work with to balance performance and efficiency.',
                  ),
                  InfoCard(
                    label: 'Governor',
                    value: _cpuInfo!['governor'] ?? 'Unavailable',
                    explanation:
                        'The algorithm that decides how fast the CPU runs at any given moment. schedutil uses the Linux scheduler to make decisions. performance always runs at maximum speed. powersave always runs at minimum speed. ondemand and interactive scale speed based on current load.',
                  ),
                  InfoCard(
                    label: 'Hardware',
                    value: _cpuInfo!['hardware'] ?? 'Unavailable',
                    explanation:
                        'The hardware platform name reported by the Linux kernel. This often corresponds to the chipset codename used internally by the manufacturer and may differ from the marketing name of the processor.',
                  ),
                  InfoCard(
                    label: 'CPU Features',
                    value: _cpuInfo!['features'] ?? 'Unavailable',
                    explanation:
                        'The instruction set extensions supported by this CPU. These are special hardware capabilities that software can use for accelerated processing. Common ones include NEON for media processing, AES for hardware encryption, and SHA for cryptographic hashing.',
                  ),
                ],
        ),
      ],
...
class InfoCardList extends StatelessWidget {
  ...
  const InfoCardList({super.key, required this.label, required this.values});
            ...
            return Padding(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(item, style: const TextStyle(fontSize: 12.0)),
                  ),
                ],
              ),
            );
